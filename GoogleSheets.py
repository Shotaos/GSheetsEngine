...
class GoogleSheets():
	...
	def get_sheets_info(self):
		response = self.service.get(spreadsheetId=self.SPREADSHEET_ID, fields='sheets/properties').execute()
	...
	def get_sheet_data(self, sheet):
		values = self.service.values().get(spreadsheetId=self.SPREADSHEET_ID, range=sheet + '!A:C').execute()
	...
	def search(self, query, sheets=[]):
		...
		values = self.service.values().batchGet(spreadsheetId=self.SPREADSHEET_ID, ranges=ranges).execute()
...
sheets = GoogleSheets()
#pprint(sheets.get_sheets_info())
#pprint(sheets.get_sheet_data('Gimp'))
pprint(sheets.search("alabama"))
